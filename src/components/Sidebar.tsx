...
import { X, Heart, Edit3, Globe, MapPin, Clock, MessageSquare, Twitter, MessageCircle, Link as LinkIcon } from 'lucide-react';
  ...
  return (
    <div className={`absolute z-[1000] transform transition-transform duration-300 ease-in-out pointer-events-none flex flex-col top-0 left-0 h-full w-full sm:top-[96px] sm:left-6 sm:h-auto sm:max-h-[calc(100vh-120px)] sm:w-[400px] ${isOpen ? 'translate-x-0' : '-translate-x-full sm:-translate-x-[calc(100%+24px)]'}`}>
      <div className="w-full h-full sm:h-auto sm:max-h-[calc(100vh-120px)] bg-white/95 backdrop-blur-2xl sm:rounded-[2rem] sm:border border-slate-200 shadow-2xl flex flex-col text-slate-800 overflow-hidden pointer-events-auto relative">
        ...
        <div className="relative h-60 sm:h-64 shrink-0 bg-slate-100 overflow-hidden">
          <div className="flex overflow-x-auto snap-x snap-mandatory no-scrollbar h-full w-full">{imagesToDisplay.map((img, idx) => (<img key={idx} src={img} alt={`${shop.name}-${idx}`} className="w-full h-full object-cover shrink-0 snap-center" />))}</div>
          {imagesToDisplay.length > 1 && (<div className="absolute top-4 left-4 bg-black/40 backdrop-blur-md px-3 py-1 rounded-full text-white text-xs font-bold pointer-events-none">1 / {imagesToDisplay.length} 左右滑動</div>)}
          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/20 to-transparent pointer-events-none" />
          <div className="absolute bottom-5 left-6 right-6 pointer-events-none"><div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold mb-2 ${isCurrentlyOpen ? 'bg-emerald-500 text-white' : 'bg-slate-500/80 text-white'}`}>{isCurrentlyOpen ? '營業中' : '休息中'}</div><h2 className="text-3xl font-extrabold text-white mb-2 tracking-wide">{shop.name}</h2><div className="flex gap-2 flex-wrap">{shop.tags?.map(tag => <span key={tag} className="px-2.5 py-1 text-xs font-bold bg-amber-500/90 text-white rounded-lg">{tag}</span>)}</div></div>
        </div>
